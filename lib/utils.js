...
function valueParameter(req, names) {
    ...
    for(let i = 0; i < names.length; i++) {
        const value = req.query[names[i]] || (req.body && req.body[names[i]]);
...
function hasFlag(req, names) {
    if(!Array.isArray(names)) names = [names];
    return names.some(name => booleanParameter(req.query[name])) || !!(req.body && names.some(name => booleanParameter(req.body[name])));
}
