...
function insertFile(ip, name, size, type, shortAlias, longAlias, revokeToken) {
    ...
    return new Promise((resolve, reject) => {
        connection.query('INSERT INTO files (uploader, name, size, type, short_alias, long_alias, revoke_token) VALUES (?, ?, ?, ?, ?, ?, ?)', [ip, name, size, type, shortAlias, longAlias, revokeToken], (error, results, fields) => {
            if(error) {
                reject(error);
            } else {
