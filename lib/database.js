...
function getArchiveFiles(type, alias) {
    ...
    return new Promise((resolve, reject) => {
        connection.query('SELECT files.id, files.name FROM files INNER JOIN archive_files ON files.id = file_id INNER JOIN archives ON archives.id = archive_id WHERE archives.?? = ?', [(type + '_alias'), alias], (error, results, fields) => {
            if(error) {
                reject(error);
            } else if(!results.length) {
